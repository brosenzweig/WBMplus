...
int MDBgcRoutingDef () {
   ...
   // Input
	if (((_MDInDischargeID           = MDDischargeDef    ()) == CMfailed) ||
	    ...
	    ((_MDNonPoint_NO3ID          = MFVarGetID (MDVarNonPoint_NO3,             "kg/m3",   MFInput, MFState, MFBoundary))  == CMfailed) ||
	    ((_MDPointSources_NO3ID      = MFVarGetID (MDVarPointSources_NO3,         "kg/day",  MFInput, MFState, MFBoundary))  == CMfailed) ||
