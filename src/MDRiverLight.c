...
static void _MDRiverLight (int itemID) {
     ...
     float par2reach;
	  ...
	  //KoppenID's : 1=Taiga, 2=semi-arid,3=tundra,4=temperate,5=tropics
	     		...
	     		if (koppen == 1) width_canopy = 2;
	     		if (koppen == 2) width_canopy = 0;
	     		if (koppen == 3) width_canopy = 0;
	     		if (koppen == 4) width_canopy = 5;
	     		if (koppen == 5) width_canopy = 10;
	     		if (isnan(koppen)) width_canopy = 5;
    ...
    if (!isnan(discharge) && (channelWidthMean > 0) && (channelWidth > 0)){
      ...
      par2reach = par2bottom * channelWidth * MFModelGetLength(itemID) * sinuosity;   //MJ/reach
